...
class YearSaleReportView(QFrame, ErrorView, ToastView):
    ...
    def set_paid_money(self, paid_money: Money):
        self.payment_money_label.setText(f' {paid_money.amount} MXN')

    def set_total_cost_money(self, total_cost: Money):
        self.total_cost_label.setText(f'-{total_cost.amount} MXN')

    def set_profit_money(self, profit: Money):
        self.profit_money_label.setText(f' {profit.amount} MXN')

    def set_total_expense_money(self, expense: Money):
        self.expense_money_label.setText(f'-{expense.amount} MXN')

    def set_net_profit(self, net_profit: Money):
        if net_profit < Money('0.00', 'MXN'):
            self.net_profit_label.setText(f'{net_profit.amount} MXN')
