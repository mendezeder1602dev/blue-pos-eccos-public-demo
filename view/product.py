from PyQt5.QtWidgets import QFrame, QMessageBox, QFileDialog, QPushButton, QLineEdit, QLabel
...
class ProductView(QFrame):
    ...
    def __setup_gui(self):
        ...
        self.__add_image_field()
        self.__add_barcode_field()
    ...
    def __add_image_field(self):
        self.image_path_edit = QLineEdit()
        self.image_path_edit.setReadOnly(True)
        browse = QPushButton('Cargar imagen')
        browse.clicked.connect(self.__choose_image)
        self.grid_form_frame.layout().addWidget(browse, 7, 0)
        self.grid_form_frame.layout().addWidget(self.image_path_edit, 7, 1)

    def __add_barcode_field(self):
        self.barcode_edit = QLineEdit()
        self.barcode_edit.setPlaceholderText('Escanea o escribe el código de barras')
        self.grid_form_frame.layout().addWidget(QLabel('Código de barras'), 6, 0)
        self.grid_form_frame.layout().addWidget(self.barcode_edit, 6, 1)

    def get_barcode(self):
        return self.barcode_edit.text().strip()

    def set_barcode(self, barcode):
        self.barcode_edit.setText(barcode or '')

    def __choose_image(self):
        path, _ = QFileDialog.getOpenFileName(self, 'Imagen del producto', '', 'Imágenes (*.png *.jpg *.jpeg *.webp)')
        if path:
    ...
    def set_image_path(self, path):
        self.image_path_edit.setText(path or '')
    ...
    def set_profit(self, amount: float):
        self.profit_value_label.setText('{} MXN'.format(str(amount)))
