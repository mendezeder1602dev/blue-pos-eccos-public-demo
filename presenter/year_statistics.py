...
class YearStatisticsPresenter(AbstractPresenter):
    ...
    def get_default_window_title(self) -> str:
        return 'Punto de Venta Grupo ECCOS - Estadísticas Anuales'
    ...
    def __plot_summaries_on_graph(self):
        ...
        revenue = sum(float(summary.acquired_money.amount) for summary in self.__month_summaries)
        expenses = sum(float(summary.total_expense.amount) for summary in self.__month_summaries)
        operations = sum(summary.sale_quantity for summary in self.__month_summaries)
        net_profit = sum(float(summary.net_profit.amount) for summary in self.__month_summaries)

        insight = (
            'El rendimiento anual mantiene una tendencia positiva y estable.'
            if net_profit >= 0 else
            'Se recomienda reducir costos para recuperar el equilibrio anual.'
        )
        self.get_view().set_period_summary(
            f'Año {self.__selected_year}',
            insight,
    ...
    def change_vertical_axis(self, selected_axis_option: str):
        ...
        if self.__month_summaries:
            ...
            self.get_view().set_period_summary(
                f'Año {self.__selected_year}',
                'Vista anual actualizada.' if net_profit >= 0 else 'Revisa la eficiencia del periodo anual.',
                {'sales': revenue, 'expenses': expenses, 'net_profit': net_profit, 'operations': operations},
            )
