...
class TestProductRepository(unittest.TestCase):
    ...
    def test_close_session_creates_fresh_repository_on_next_access(self):
        product_repository = RepositoryFactory.get_product_repository(TEST_DB_URL)

        RepositoryFactory.close_session()

        refreshed_repository = RepositoryFactory.get_product_repository(TEST_DB_URL)

        self.assertIsNot(product_repository, refreshed_repository)
        self.assertIsNotNone(RepositoryFactory._RepositoryFactory__session)

    def test_active_workspace_is_kept_when_repository_url_is_omitted(self):
        RepositoryFactory.activate_workspace(TEST_DB_URL)

        product_repository = RepositoryFactory.get_product_repository()
        sale_repository = RepositoryFactory.get_sale_repository()

        session = RepositoryFactory._RepositoryFactory__session
        self.assertIs(product_repository._ProductRepository__session, session)
        self.assertIs(sale_repository._SaleRepository__session, session)

    def test_switching_database_recreates_cached_repositories(self):
        first_repository = RepositoryFactory.get_product_repository(TEST_DB_URL)

        RepositoryFactory.get_sale_repository('sqlite:///:memory:')
        second_repository = RepositoryFactory.get_product_repository()
